...
class HSIDataset(CustomDataset):
    ...
    def __init__(self,
        ...
        if self.classwise:
            HSIDataset.PALETTE = HSIDataset.CLASSWISE_PALETTE
            HSIDataset.CLASSES = self.HRSC_CLASSES
            self.catid2label = {
                ('1' + '0' * 6 + cls_id): i
                for i, cls_id in enumerate(self.HRSC_CLASSES_ID)
            }
        else:
            HSIDataset.CLASSES = self.HRSC_CLASS
        # self.cat2label = {cat: i for i, cat in enumerate(self.CLASSES)}

        # load_annotations
        super(HSIDataset, self).__init__(ann_file, pipeline, **kwargs)
        ...
        for img_id in img_ids:
            ...
            if gt_bboxes:
                ...
                data_info['ann']['headers'] = np.zeros((0, 2),
                                                       dtype=np.float32)
            #gt_polygons_ignore标记需要忽略的目标区域，比如难以识别或遮挡等。
