...
class ISDDDataset(CustomDataset):
    CLASSES = ('ship', )
    def __init__(self,
                 ann_file,
                 pipeline,
        ...
        # load_annotations
        super(ISDDDataset, self).__init__(ann_file, pipeline, **kwargs)
    ...
    def load_annotations(self, ann_file):
        ...
        for img_id in img_ids:
            ...
            data_info['filename'] = f'{img_id}.png'
            ...
            for obj in root.findall('object'):
                ...
                # bbox shape(1,6)
                bbox = np.array([[
                    float((float(obj.find('bndbox/xmin').text) + float(obj.find('bndbox/xmax').text)) / 2),
                    float((float(obj.find('bndbox/ymin').text) + float(obj.find('bndbox/ymax').text)) / 2),
                    float(float(obj.find('bndbox/xmax').text) - float(obj.find('bndbox/xmin').text)),
                    float(float(obj.find('bndbox/ymax').text) - float(obj.find('bndbox/ymin').text)),
                    float(0), 0
                ]],
                                dtype=np.float32)
                ...
                # 表示目标的头部坐标
                head = np.array([
                    int((float(obj.find('bndbox/xmin').text) + float(obj.find('bndbox/xmax').text)) / 2),
                    int((float(obj.find('bndbox/ymin').text) + float(obj.find('bndbox/ymax').text)) / 2),
                ],
                                dtype=np.int64)
